...
AIFF_ReadRef AIFF_Open(const char* file)
{
	AIFF_ReadRef r ;
	IFFHeader hdr ;
...
char* AIFF_GetAttribute(AIFF_ReadRef r,IFFType attrib)
{
	switch( r->format )
	{
		case AIFF_TYPE_AIFF:
...
int AIFF_ReadMarker(AIFF_ReadRef r,int* id,uint32_t* pos,char** name)
{
	switch( r->format )
	{
		case AIFF_TYPE_AIFF:
...
int AIFF_GetInstrumentData(AIFF_ReadRef r,Instrument* i)
{
	switch( r->format )
	{
		case AIFF_TYPE_AIFF:
...
int AIFF_GetSoundFormat(AIFF_ReadRef r,uint32_t* nSamples,int* channels,
...
size_t AIFF_ReadSamples(AIFF_ReadRef r,void* buffer,size_t len)
{
	int res = 1 ;
...
int AIFF_Seek(AIFF_ReadRef r,uint32_t pos)
{
	int res = 0 ;
...
int AIFF_ReadSamples32Bit(AIFF_ReadRef r,int32_t* samples,int nsamples)
{
	void* buffer;
	int i,j ;
	size_t h ;
...
void AIFF_Close(AIFF_ReadRef r)
{
	if( r->buffer )
		free( r->buffer ) ;
	fclose( r->fd ) ;
...
AIFF_WriteRef AIFF_WriteOpen(const char* file)
{
	AIFF_WriteRef w ;
	IFFHeader hdr ;
...
int AIFF_StartWritingSamples(AIFF_WriteRef w)
{
	IFFChunk chk ;
	SoundChunk s ;
...
static void* InitBuffer(AIFF_WriteRef w, size_t len)
{
	if( w->buflen < len )
	{
modsize:
...
static void DestroyBuffer(AIFF_WriteRef w)
{
	if( w->buffer )
		free( w->buffer );
...
int AIFF_WriteSamples(AIFF_WriteRef w,void* samples,size_t len)
{
	int n;
	void* buffer;
...
static int AIFF_WriteSamplesNoSwap(AIFF_WriteRef w,void* samples,size_t len)
{
...
int AIFF_WriteSamples32Bit(AIFF_WriteRef w,int32_t* samples,int nsamples)
{
	register int i, j ;
...
int AIFF_EndWritingSamples(AIFF_WriteRef w)
{
	char car = 0 ;
	uint32_t segment ;
	long of ;
...
int AIFF_StartWritingMarkers(AIFF_WriteRef w)
{
	IFFChunk chk ;
	uint16_t nMarkers = 0 ;
...
int AIFF_WriteMarker(AIFF_WriteRef w,uint32_t position,char* name)
{
	Marker m ;
	int l = 0 ;
	int pad = 0 ;
...
int AIFF_EndWritingMarkers(AIFF_WriteRef w)
{
	IFFType typ ;
	IFFType ckid ;
	uint32_t cklen ;
...
int AIFF_WriteClose(AIFF_WriteRef w)
{
	int ret = 1 ;
	IFFType typ ;
	IFFHeader hdr ;
